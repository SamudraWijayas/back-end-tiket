...
export default {
  // Sales Stats per bulan
  async salesStats(req: IReqUser, res: Response) {
    try {
      const userId = req.user?.id;
      if (!userId) return response.error(res, null, "user not found");

      const { eventId, year } = req.query;

      const query: any = { status: OrderStatus.COMPLETED };

      // filter event
      if (eventId && typeof eventId === "string") {
        query.events = new mongoose.Types.ObjectId(eventId);
      }

      // filter tahun
      if (year && !isNaN(Number(year))) {
        const start = new Date(Number(year), 0, 1); // 1 Jan
        const end = new Date(Number(year) + 1, 0, 1); // 1 Jan tahun berikutnya
      ...
      response.success(res, stats, "success get sales stats");
    } catch (error) {
      response.error(res, error, "failed to get sales stats");
    }
  },
  // Ticket Stats per event/ticket
  async ticketStats(req: IReqUser, res: Response) {
    try {
      ...
      const matchStage: any = { status: OrderStatus.COMPLETED };
      if (eventId && typeof eventId === "string") {
        matchStage.events = new mongoose.Types.ObjectId(eventId);
      }
  ...
  async totalIncomeByOrganizer(req: IReqUser, res: Response) {
    try {
      ...
      const query: any = { status: OrderStatus.COMPLETED };
      if (eventId) query.events = eventId;
